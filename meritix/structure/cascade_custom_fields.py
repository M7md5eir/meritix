...
Whitelisted endpoints (used by the Structure Level form):
    search_meritix_doctypes(...)  -- filter the target_doctype picker
        to DocTypes shipped by the meritix app.
...
def fill(doc, target_doctype: str) -> None:
	...
	Resets each cascade fieldname to ``None`` first so an organization
	reparented to a shallower branch doesn't leave a stale value pinned to a
	level it no longer belongs to.
	"""
...
def search_meritix_doctypes(
	...
	Returns only DocTypes shipped by the meritix app and excludes child
